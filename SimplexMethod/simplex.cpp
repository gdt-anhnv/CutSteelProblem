...
double C[N]={-6,-5,-4,0,0,0};  //Initialize the C array  with the coefficients of the constraints of the objective function
double B[M]={240,360,300};//Initialize the B array constants of the constraints respectively
...
void Simplex::doPivotting(int pivotRow, int pivotColumn) {
	...
	std::vector<double> pivotRowVals = std::vector<double>(cols, 0);
	//double pivotRowVals[cols];//the column with the pivot

	std::vector<double> pivotColVals = std::vector<double>(rows, 0);
	//double pivotColVals[rows];//the row with the pivot

	std::vector<double> rowNew = std::vector<double>(cols, 0);
...
{
	std::vector<double> positiveValues = std::vector<double>(rows, 0);
